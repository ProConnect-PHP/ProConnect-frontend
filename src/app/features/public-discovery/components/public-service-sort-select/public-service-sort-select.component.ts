...
@Component({
  ...
  template: `
    <label class="block text-sm font-semibold text-slate-800" for="public-service-sort">
      Ordenar
    </label>
    <select
      id="public-service-sort"
      class="mt-2 min-h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 shadow-sm outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100"
      [value]="sort()"
